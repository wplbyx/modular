...
`packages/infra/cache/redis`: `redis.NewRedisClient(cfg *config.Redis) (redis.UniversalClient, error)`. Universal client: pass `Urls` for a cluster/sentinel, or `Host`+`Port` for a single node. Pings on connect. For Application wiring use `redis.NewResource(cfg)`; after setup read the client with `resource.Client()`. Extras: `redis` package also has `bloom.go` (Bloom filter via Lua scripts) and `idempotence.go`. Sets a package global `globalClient` for compatibility.
