...
Use this workflow for DB, Redis, Storage, Telemetry, or local EventBus wiring.
...
2. Choose the library Resource constructor: Bun, GORM plus dialect, Mongo,
   Redis, storage resource, OpenTelemetry, or EventBus.
3. Keep resources in Application lifecycle order and expose typed providers to
   ...
   EventBus is opt-in and exposes `*eventbus.Bus`; selecting it must not create
   handlers, domain events, or other business shells.
