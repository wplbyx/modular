...
type Application struct {
	...
	InstanceID      string            `mapstructure:"InstanceID"`                                   // 进程实例ID，为空时按Transport派生
	Metadata        map[string]string `mapstructure:"Metadata"`                                     // 进程注册元数据
...
func (Application) Flags(prefix string) []FlagSpec {
	return []FlagSpec{
		...
		{Name: flagName(prefix, "InstanceID"), Default: "", Usage: "进程实例ID"},
